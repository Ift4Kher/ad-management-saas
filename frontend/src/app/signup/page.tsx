...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      signup(data.token, data.user, data.workspace);
      ...
      <main className="flex flex-1 items-center justify-center px-4 sm:px-6 py-12">
        <div className="w-full max-w-md rounded-[var(--radius-xl)] border border-neutral-200 bg-surface p-6 sm:p-8 shadow-sm">
          <div className="text-center">
            <h1 className="text-xl font-bold text-neutral-900">{t('auth.signup_title')}</h1>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-neutral-700">{t('auth.name_label')}</label>
              <input
                type="text"
                required
            ...
            >
              {submitting ? t('auth.creating_account') : t('cta.get_started')}
            </button>
          </form>
